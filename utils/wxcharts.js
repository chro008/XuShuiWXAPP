...
var config = {
  ...
  colors: ["#556fb5", "#f3901d", "#f19ec2", "#e9d601", "#aa89bd", "#009e96", "#448aca", "#f68475", "#77a1e5",
    "#af4e23", "#09ccda", "#979797"],
  ...
  series.forEach(function (eachSeries, seriesIndex) {
    ...
    splitPointList.forEach(function (points, index) {
      ...
      if (points.length === 1) {
        ...
        context.arc(points[0].x, points[0].y, 0.5, 0, 2 * Math.PI);
      } else {
        context.moveTo(points[0].x, points[0].y);
        if (opts.extra.lineStyle === 'curve') {
          points.forEach(function (item, index) {
...
function drawYAxisGrid(opts, config, context) {
  ...
  context.beginPath();
  context.setStrokeStyle(opts.yAxis.gridColor || "#e5e5e5");
...
function drawYAxis(series, opts, config, context) {
  ...
  context.beginPath();
  context.setFontSize(config.fontSize);
  context.setFillStyle(opts.yAxis.fontColor || '#999999');
